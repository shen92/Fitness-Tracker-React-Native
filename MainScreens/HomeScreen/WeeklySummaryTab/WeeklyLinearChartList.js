...
class WeeklyLinearChartList extends Component {
    ...
    getChartData() {
        let calories = [];
        let nutrition = [];
        if (this.state.caloriesList.length !== 0 && this.state.nutritionList.length != 0) {
            for (let i = 0; i < 7; i++) {
                calories.push({
    ...
    getChartList() {
        let charts = [];
        charts.push(<WeeklyLinearChart key={1} title={'Calories'} data={this.state}/>);
        charts.push(<WeeklyLinearChart key={2} title={'Nutrition'} data={this.state}/>);
        return charts;
    }
