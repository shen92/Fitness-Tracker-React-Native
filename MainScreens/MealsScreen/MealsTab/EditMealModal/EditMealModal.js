...
class EditMealModal extends Component {
    ...
    pullFoodsData() {
        ...
        let url = "https://mysqlcs639.cs.wisc.edu/meals/" + this.props.value.id + "/foods"
    ...
    removeFoodData(id) {
        ...
        let url = "https://mysqlcs639.cs.wisc.edu/meals/" + this.props.value.id + "/foods/" + id;
        fetch(url, requestOptions).then(
            res => res.json()
        ).then(result => {
        ...
        return (
            <React.Fragment>
                ...
                <View style={local.modalBottom}>
                    <Button title={"cancel"} onPress={() => this.props.hideEditMealModal()}/>
                </View>
            </React.Fragment>
        );
    }
